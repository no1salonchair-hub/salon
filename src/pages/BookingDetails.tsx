    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          >
            <div className="text-center">
              <div className="w-20 h-20 bg-purple-500/20 rounded-2xl flex items-center justify-center mx-auto mb-4 text-purple-400">
                <Scissors className="w-10 h-10" />
              </div>
              <div className="space-y-1">
                {(booking.services || ((booking as any).service ? [(booking as any).service] : [])).map((service, idx) => (
                  <h2 key={idx} className="text-xl font-bold text-white">{service}</h2>
                ))}
              </div>
              <span className={cn(
            ...
            <div className="space-y-4 pt-6 border-t border-white/10">
              <div className="flex items-center gap-3 text-white/40">
                ...
                <div>
                  <p className="text-xs uppercase tracking-widest font-bold text-white/40">Date</p>
              ...
              <div className="flex items-center gap-3 text-white/40">
                ...
                <div>
                  <p className="text-xs uppercase tracking-widest font-bold text-white/40">Time</p>
              ...
              <div className="flex items-center gap-3 text-white/40">
                ...
                <div className="flex-1">
                  <p className="text-xs uppercase tracking-widest font-bold text-white/40">Salon</p>
                ...
                {salon.location && typeof salon.location.lat === 'number' && (
                  <button
                    ...
                    className="p-2 bg-purple-500/20 text-purple-400 rounded-xl hover:bg-purple-500/30 transition-all border border-purple-500/20 shadow-lg"
                    title="Get Directions"
                  >
                    <MapPin className="w-4 h-4 fill-purple-400/20" />
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="lg:col-span-2">
          <div
            className="bg-white/5 border border-white/10 rounded-3xl flex flex-col h-[600px] overflow-hidden shadow-xl"
          >
            <div className="p-6 border-b border-white/10 flex items-center justify-between bg-white/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/40">Real-time chat with {profile?.role === 'user' ? 'Salon Owner' : 'Customer'}</p>
                </div>
              </div>
            </div>
            ...
            {booking.status === 'accepted' ? (
              <>
                <div
                  ref={scrollRef}
                  className="flex-1 overflow-y-auto p-6 space-y-4 scroll-smooth bg-black/20"
                >
                  {messages.map((msg) => (
                    <div
                      key={msg.id}
                    ...
                    >
                      ...
                      <span className="text-[9px] text-white/40 mt-1 px-1 font-bold uppercase tracking-widest">
                ...
                <div className="p-4 bg-white/5 border-t border-white/10">
                  <form onSubmit={sendMessage} className="flex gap-3">
                    <input
                      ...
                      className="flex-1 px-6 py-4 bg-white/5 border border-white/10 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all text-white placeholder:text-white/20"
                    />
                    <button
                      type="submit"
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-10 text-center bg-black/20">
                <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mb-4">
                  <MessageCircle className="w-8 h-8 text-white/20" />
                </div>
                <h4 className="text-xl font-bold text-white/40">Chat Locked</h4>
